...
v_day <- c(21)
v_treat <- c("MB_Clostridia","Clostridia_only")
...
deseq_main <- function(d=21,t=c("MB_Clostridia","Mb_only"),v="Treatment",physeq_object=phyloseq_object_final){
  ...
  pointplot <- deseq_pointplot(d,t,v,deseq_results_filtered) + theme_bw() +
...
```
# Ok so that is not working. Let's do it stepwise without functions.
```{r }
load("phyloseq_object_final.RData")
load("tax_matrix.RData")
...
d=21
t=c("MB_Clostridia","Clostridia_only")
...
deseq_processing <- function(d=21,t=c("MB_Clostridia","Mb_only"),v="Treatment",physeq_object=phyloseq_object_final){
  #Filter the phyloseq object by input variable and glom 
